...
export async function GET(req: NextRequest) {
  ...
  const ids = adIds.split(",").slice(0, 50)
      ...
      ids.map(async (adId) => {
        try {
          const res = await fetch(
            `${META_API}/${adId}?fields=id,name,account_id,creative{thumbnail_url,image_url,object_story_spec}&access_token=${creds.access_token}`
          )
          if (!res.ok) return { ad_id: adId, thumbnail_url: null, image_url: null }
          const json = await res.json()

          const accountRef = json.account_id ? `act_${json.account_id}` : null
          if (!accountRef || !access.allowedAccountIds.has(accountRef)) {
