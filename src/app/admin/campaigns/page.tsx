...
export default async function AdminCampaignsPage({ searchParams }: PageProps) {
  ...
  const [
    ...
    supabase
      .from("campaigns")
      .select("id, name, channel, sync_tag, start_date, end_date, brand_id")
    ...
    supabase.from("meta_ad_accounts").select("brand_id"),
    supabase.from("naver_ad_accounts").select("brand_id"),
    supabase.from("spend_records").select("campaign_id, amount"),
  ])

  const activeBrandId = selectedBrandId ?? brands?.[0]?.id ?? ""

  const campaigns = allCampaigns?.filter((c) => c.brand_id === activeBrandId) ?? []
  const hasMeta = (metaAccounts ?? []).some((m) => m.brand_id === activeBrandId)
  const hasNaver = (naverAccounts ?? []).some((n) => n.brand_id === activeBrandId)

  const spendTotals: Record<string, number> = {}
  for (const s of spends ?? []) {
    spendTotals[s.campaign_id] = (spendTotals[s.campaign_id] ?? 0) + Number(s.amount ?? 0)
  }
