...
const TABS = [
  ...
  { id: "engagement", label: "Engagement", count: "12" },
  { id: "conversions", label: "Conversions", count: "6" },
  { id: "retention", label: "Retention" },
  { id: "explorer", label: "User Explorer" },
  ...
  return (
    <>
      ...
      <div className="canvas">
        {show(["acquisition", "engagement", "conversions"]) && (
        ...
        {show(["explorer"]) && <UserExplorer />}

        {show(["engagement", "acquisition"]) && (
          <div className="three">
            <div className="panel">
              <div className="p-head">
                <h3>Events Stream</h3>
            ...
            <div className="panel">
              ...
              <div className="p-body">
                <div style={{ fontSize: 10, color: "var(--dim)", textTransform: "uppercase", letterSpacing: ".12em", marginBottom: 8 }}>
                  Device
                </div>
                ...
                <div style={{ fontSize: 10, color: "var(--dim)", textTransform: "uppercase", letterSpacing: ".12em", margin: "18px 0 8px" }}>
                  Browser
                </div>
        ...
        {tab === "retention" && (
          <div className="panel">
            <div className="p-head">
              ...
              <div className="sub">Weekly cohorts · 8 weeks</div>
              <TabGroup tabs={["User", "Revenue"]} initial="User" />
            </div>
            <div className="p-body">
              <div className="aud" style={{ gridTemplateColumns: "80px repeat(8,1fr)" }}>
                <div className="lbl" />
                {["Wk 0", "Wk 1", "Wk 2", "Wk 3", "Wk 4", "Wk 5", "Wk 6", "Wk 7"].map((h) => (
                  <div key={h} className="hd">{h}</div>
                ))}
                {RETENTION.map((r) => (
                  <Fragment key={r.week}>
                    <div className="lbl">{r.week}</div>
                    {r.vals.map((v, i) => (
                      <div
                        key={i}
                        className="c"
                        style={{
                          background: v ? `rgba(125,184,214,${v / 100 * 0.8})` : "var(--bg-2)",
                          color: v && v > 40 ? "#0A0B0E" : undefined,
                        }}
                      >
                        {v ? <><span className="v">{v}%</span></> : <span style={{ color: "var(--dimmer)" }}>—</span>}
                      </div>
                    ))}
                  </Fragment>
                ))}
              </div>
              <div style={{ fontSize: 10, color: "var(--dim)", marginTop: 14, textAlign: "center" }}>
                Avg Week 1 retention: <b style={{ color: "var(--steel)" }}>18.4%</b> · Week 4: <b style={{ color: "var(--steel)" }}>8.2%</b>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "reports" && (
          <div className="panel">
            <div className="p-head">
              <h3>Saved Reports</h3>
              <div className="sub">12 reports</div>
              <button className="more">···</button>
            </div>
            <div className="p-body" style={{ paddingTop: 8 }}>
              {REPORTS.map((r, i) => (
                <div key={i} className="log-row">
                  <div className={`d ${r.kind}`} />
                  <div className="who">{r.author}<span className="when">{r.date}</span></div>
                  <div className="what">{r.title}</div>
                  <div className="act">{r.type}</div>
                </div>
              ))}
            </div>
          </div>
...
const RETENTION = [
  { week: "Mar 3", vals: [100, 22, 14, 10, 8, 6, 5, 4] },
  { week: "Mar 10", vals: [100, 24, 16, 12, 9, 7, 5, null] },
  { week: "Mar 17", vals: [100, 19, 12, 8, 7, 5, null, null] },
  { week: "Mar 24", vals: [100, 18, 11, 8, 6, null, null, null] },
  { week: "Mar 31", vals: [100, 16, 10, 7, null, null, null, null] },
  { week: "Apr 7", vals: [100, 14, 9, null, null, null, null, null] },
]

const REPORTS = [
  { kind: "", author: "Henry Park", date: "Apr 12", title: "Monthly Performance — March 2026", type: "Scheduled" },
  { kind: "ai", author: "BP360 Agent", date: "Apr 10", title: "Creative fatigue analysis — Q1 retrospective", type: "AI · Auto" },
  { kind: "", author: "Jiwoo Lee", date: "Apr 8", title: "Audience expansion test results — Lookalike 5%", type: "Manual" },
  { kind: "sys", author: "System", date: "Apr 7", title: "Weekly budget reconciliation", type: "Scheduled" },
  { kind: "ai", author: "BP360 Agent", date: "Apr 5", title: "Conversion path analysis — top 3 journeys", type: "AI · Auto" },
  { kind: "", author: "Henry Park", date: "Apr 1", title: "Q1 2026 comprehensive report", type: "Manual" },
]
